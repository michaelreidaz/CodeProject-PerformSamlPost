...
namespace davidsp8.windows.PostSamlAssertion {
    public partial class frmMain : Form {
        ...
        /// <summary>
        /// SAML that has been viewed
        /// </summary>
        private string viewedSaml = String.Empty;
        /// <summary>
        ...
        private void updateConfigurationFileFromForm() {
            ...
            if (this.rbSignResponse.Checked) {
                configurationData.Configuration.Rows[0]["SignatureType"] = SigningHelper.SignatureType.Response.ToString();
            } else {
                configurationData.Configuration.Rows[0]["SignatureType"] = SigningHelper.SignatureType.Assertion.ToString();
            }
        }
        /// <summary>
        ...
        private void updateFormFromConfigurationFile() {
            ...
            this.lblEncryptionCertificateFriendlyName.Text = configurationData.Configuration.Rows[0]["EncFriendlyName"].ToString();
            ...
            if (configurationData.Configuration.Rows[0]["SignatureType"] != null) {
                signatureType = (SigningHelper.SignatureType)Enum.Parse(typeof(SigningHelper.SignatureType), configurationData.Configuration.Rows[0]["SignatureType"].ToString());
            }
            if (signatureType == SigningHelper.SignatureType.Response) {
                this.rbSignResponse.Checked = true;
            } else {
                this.rbSignAssertion.Checked = true;
            }
        ...
        private void btnPostAssertion_Click(object sender, EventArgs e) {
            try {
                ...
                string encCertFileLocation = "";
                
                if (configurationData.Configuration.Rows[0]["EncCertFileLocation"] != null) {
                    encCertFileLocation = configurationData.Configuration.Rows[0]["EncCertFileLocation"].ToString().Length > 0 ?
                                     configurationData.Configuration.Rows[0]["EncCertFileLocation"].ToString() : null;
                }
                ...
                // Get SAML Post Value
                if (rb_v1.Checked) {
                    saml = davidsp8.common.Security.Saml11.SamlHelper.GetPostSamlResponse(txtRecipient.Text,
                    ...
                    postData = String.Format("SAMLResponse={0}&TARGET={1}",
                        System.Web.HttpUtility.UrlEncode(saml),txtTarget.Text);
                }
                if (rb_v2.Checked) {
                    if (encCertFileLocation == null) {
                        saml = davidsp8.common.Security.Saml20.SamlHelper.GetPostSamlResponse(txtRecipient.Text,
                                txtIssuer.Text, txtDomain.Text, txtSubject.Text,
                                storeLocation, storeName, findType,
                                certFileLocation, certPassword, certFindKey,
                                attributes, signatureType);
                        ...
                        saml = davidsp8.common.Security.Saml20.SamlHelper.GetPostSamlResponse(txtRecipient.Text,
                                ...
                                storeLocation, storeName, findType,
                                certFileLocation, certPassword, certFindKey,
                                attributes, signatureType, encCertFileLocation);
                    }
                    if (nonStandardParms) {
                        StringBuilder sb = new StringBuilder();
                        foreach (ConfigurationData.AttributesRow row in configurationData.Attributes.Rows) {
                            if (row.AttrPostSaml == "Post") {
                                sb.AppendFormat("{0}={1}&", row.Name, row.Value);
                            }
                            if (row.AttrPostSaml == "Saml") {
                                sb.AppendFormat("SAMLResponse={0}&", System.Web.HttpUtility.UrlEncode(saml));
                            }
                        }
                        postData = sb.ToString().TrimEnd('&');
                    } else {
                        postData = String.Format("SAMLResponse={0}&RelayState={1}",
                            System.Web.HttpUtility.UrlEncode(saml),
                ...
                tabControl1.SelectedTab = tpBrower;
        ...
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e) {
            ...
            if (viewedSaml != String.Empty) {
                File.Delete(viewedSaml);
            }
        }
        /// <summary>
        ...
        private void btnShowSaml_Click(object sender, EventArgs e) {
            

        }

        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e) {
            if (tabControl1.SelectedTab == tpSaml) {
                if (viewedSaml != String.Empty) {
                    File.Delete(viewedSaml);
                }
                viewedSaml = Path.GetTempFileName() + ".xml";
                string decodedString = Encoding.UTF8.GetString(Convert.FromBase64String(saml));
                File.WriteAllText(viewedSaml, decodedString);
                samlBrowser.Url = new Uri("file://" + viewedSaml);
                samlBrowser.Visible = true;
            }
        }

        private void lblCertificate_Click(object sender, EventArgs e) {

        }

        private void btnSelectEncryptionCertificate_Click(object sender, EventArgs e) {
            frmSelectCertificate cert = new frmSelectCertificate(configurationData, true);
            cert.ShowDialog(this);
            if (cert.DialogResult == DialogResult.OK) {
                this.lblEncryptionCertificateFriendlyName.Text =
                    configurationData.Configuration.Rows[0]["EncFriendlyName"].ToString();
            }
        }

        private void attributesBindingSource_CurrentChanged(object sender, EventArgs e) {
